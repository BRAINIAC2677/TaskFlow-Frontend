<div
    class="grid grid-cols-1 gap-4 p-4 mb-4 rounded-lg overview-section md:grid-cols-4"
>
    <!-- Task Completion Rate Card -->
    <div class="p-4 bg-accent-50 dark:bg-accent-900 rounded-lg shadow card">
        <h3 class="text-lg font-semibold">Task Completion Rate</h3>
        <p class="text-2xl font-bold">200/300 Tasks</p>
        <p class="text-accent-600 dark:text-accent-200">
            67% of tasks completed
        </p>
    </div>

    <!-- Active Tasks Card -->
    <div class="p-4 bg-accent-50 dark:bg-accent-900 rounded-lg shadow card">
        <h3 class="text-lg font-semibold">Active Boards</h3>
        <p class="text-2xl font-bold">10</p>
        <p class="text-accent-600 dark:text-accent-200">
            boards currently active
        </p>
    </div>

    <!-- Upcoming Deadlines Card -->
    <div class="p-4 bg-accent-50 dark:bg-accent-900 rounded-lg shadow card">
        <h3 class="text-lg font-semibold">Upcoming Deadlines</h3>
        <p class="text-2xl font-bold">8</p>
        <p class="text-accent-600 dark:text-accent-200">Tasks due this week</p>
    </div>

    <!-- Personal Goal Card -->
    <div class="p-4 bg-accent-50 dark:bg-accent-900 rounded-lg shadow card">
        <h3 class="text-lg font-semibold">Daily Goals Progress</h3>
        <p class="text-2xl font-bold">6/10 Tasks</p>
        <p class="text-accent-600 dark:text-accent-200">
            60% of daily goal completed
        </p>
    </div>
</div>
