    ...
    <div class="absolute w-full mt-1">
        ...
        {:else if filteredSuggestions.length === 0 && searchTerm !== ""}
            <div class="absolute z-10 w-full bg-white shadow-lg">
                <div class="px-4 py-2 text-gray-700 cursor-default select-none">
                    No match found
                </div>
            </div>
        {:else}
            <ul class="absolute z-10 w-full bg-white shadow-lg">
                {#each filteredSuggestions as suggestion (suggestion.id)}
                    <!-- svelte-ignore a11y-click-events-have-key-events -->
                    <!-- svelte-ignore a11y-no-noninteractive-element-interactions -->
                    <li
                        class="px-4 py-2 cursor-default select-none hover:bg-gray-100"
                        on:click={() =>
                            selectSuggestion(suggestion.id, suggestion.name)}
                    >
